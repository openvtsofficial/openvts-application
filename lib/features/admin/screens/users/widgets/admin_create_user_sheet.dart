...
class AdminCreateUserSheet extends ConsumerStatefulWidget {
  const AdminCreateUserSheet({super.key});

  @override
  ConsumerState<AdminCreateUserSheet> createState() =>
      _AdminCreateUserSheetState();
}

class _AdminCreateUserSheetState extends ConsumerState<AdminCreateUserSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isCatalogLoading && _countries.isEmpty) {
      return const Center(child: OpenVtsLoader());
    }

    if (_catalogError != null && _countries.isEmpty) {
    ...
    return _buildForm(context, usersState.isCreating);
  }
